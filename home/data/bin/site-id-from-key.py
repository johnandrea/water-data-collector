...
if len(sys.argv) < 2:
   ...
   sys.exit( 1 )
if len(sys.argv) < 3:
   print( 'Missing site key as second parameter', file=sys.stderr )
   sys.exit( 1 )

dbfile = sys.argv[1]
site_key = sys.argv[2].lower()

if not os.path.isfile( dbfile ):
   print( 'Db config file does not exist', file=sys.stderr )
   sys.exit( 1 )
if not site_key:
   print( 'Site key is empty', file=sys.stderr )
   print( '' )
   sys.exit( 1 )
...
try:
   ...
   cur.execute( 'select id from sites where web_key=%s', (site_key,) )
